...
# Defaults alinhados ao range observado no scaler de treino.
DEFAULT_MONTH_FOR_MODEL = 9
DEFAULT_IS_HOLIDAY_FOR_MODEL = 1
...
async def executar_previsao(vazao_atual: float, pressao_atual: float, lat: float, lon: float):
    ...
    humidity = float(clima["humidity"])
    mes_sen_default = round(math.sin(2 * math.pi * DEFAULT_MONTH_FOR_MODEL / 12), 4)
    mes_cos_default = round(math.cos(2 * math.pi * DEFAULT_MONTH_FOR_MODEL / 12), 4)
    ...
    lags = {
        ...
        "temp_lag_1h": temp - 1.0
    }

    vetor_final = [
        pressao_atual,
        t["dia_da_semana"],
        DEFAULT_IS_HOLIDAY_FOR_MODEL,
        t["hora_sen"],
        t["hora_cos"],
